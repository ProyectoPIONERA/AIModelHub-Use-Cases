...
def _make_wh_metrics_endpoint(model_name: str):
    def endpoint(requests: List[WHMetricSample]):
        return flares_models.evaluate_wh_batch(model_name, requests)
    return endpoint
...
def _make_reliability_metrics_endpoint(model_name: str):
    def endpoint(requests: List[ReliabilityMetricSample]):
        return flares_models.evaluate_reliability_batch(model_name, requests)
    return endpoint


for model_name, model_type in flares_models.model_types.items():
    route = f"/flares/{model_name}"
    metrics_route = f"{route}/metrics"
    if model_type == "5w1h":
        app.add_api_route(route, _make_wh_endpoint(model_name), methods=["POST"], summary=f"5W1H model: {model_name}")
        app.add_api_route(
            metrics_route,
            _make_wh_metrics_endpoint(model_name),
            methods=["POST"],
            summary=f"5W1H metrics: {model_name}",
        )
    else:
        app.add_api_route(route, _make_reliability_endpoint(model_name), methods=["POST"], summary=f"Reliability model: {model_name}")
        app.add_api_route(
            metrics_route,
            _make_reliability_metrics_endpoint(model_name),
            methods=["POST"],
            summary=f"Reliability metrics: {model_name}",
        )
